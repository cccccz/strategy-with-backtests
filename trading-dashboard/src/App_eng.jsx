    ...
    return (
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-l-4 border-blue-500 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600">Buy @ {opp.best_buy_exchange}</p>
            <p className="text-lg font-mono text-green-600">{formatPrice(opp.best_buy_price)}</p>
            <p className="text-sm text-gray-600">Sell @ {opp.best_sell_exchange}</p>
    ...
    return (
      <div className="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            <p className="text-sm text-gray-600">Buy @ {pos.best_buy_exchange}</p>
            <p className="text-lg font-mono text-green-600">{formatPrice(pos.best_buy_price)}</p>
          </div>
          
          <div>
            <p className="text-sm text-gray-600">Sell @ {pos.best_sell_exchange}</p>
            <p className="text-lg font-mono text-red-600">{formatPrice(pos.best_sell_price)}</p>
          </div>
          
          <div>
            <p className="text-sm text-gray-600">Open Spread</p>
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border p-6">
        ...
        {balance.exchange_balances && Object.keys(balance.exchange_balances).length > 0 && (
          <div>
            <h4 className="font-medium text-gray-700 mb-3">Exchange Allocation</h4>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="text-left p-3">Buy@Exchange</th>
                <th className="text-left p-3">Sell@Exchange</th>
            ...
            <tbody>
              {trade_pairs.map((trade, index) => {
                const buyInfo = `${trade.buy_exchange?.slice(0, 3).toUpperCase()}@${formatPrice(trade.open_buy_price)}${trade.close_buy_price ? `/${formatPrice(trade.close_buy_price)}` : trade.current_buy_price ? `/${formatPrice(trade.current_buy_price)}` : '/N/A'}`;
                const sellInfo = `${trade.sell_exchange?.slice(0, 3).toUpperCase()}@${formatPrice(trade.open_sell_price)}${trade.close_sell_price ? `/${formatPrice(trade.close_sell_price)}` : trade.current_sell_price ? `/${formatPrice(trade.current_sell_price)}` : '/N/A'}`;
                ...
                return (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-3">{trade.close_time ? formatTime(trade.close_time) : 'OPEN'}</td>
        ...
        {summary && (
          <div className="mt-6 p-4 bg-gray-50 rounded border-t">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div className="text-center">
                <p className="text-gray-600">Open Trades</p>
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Arbitrage Trading Dashboard</h1>
          <p className="text-gray-600 mt-2">Real-time monitoring of cross-exchange trading opportunities</p>
        </div>
