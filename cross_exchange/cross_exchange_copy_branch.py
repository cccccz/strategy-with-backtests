...
class TradingState:
    ...
    async def update_redis_data(self):
        ...
        try:
            ...
            redis_client.set("trading:exchange_data", json.dumps(self.shared_data))
            
            # 2. Current active position (from display_trade)
            if self.active_trades:
                current_position = self.active_trades[0]  # Assuming single position
                redis_client.set("trading:current_position", json.dumps(current_position))
            else:
                redis_client.delete("trading:current_position")
            
            # 3. Formatted trade history (matching display_trade_history logic)
            formatted_history = self._format_trade_history()
            redis_client.set("trading:formatted_history", json.dumps(formatted_history))
            ...
            redis_client.set("trading:balance", json.dumps(balance_info))
            
            # 5. Latest opportunity with spread info
            if self.latest_opportunity:
                redis_client.set("trading:latest_opportunity", json.dumps(self.latest_opportunity))
            ...
            for key, value in metrics.items():
                redis_client.hset("trading:metrics", key, value)
...
def prepare_exchange_data(state):
    return 
